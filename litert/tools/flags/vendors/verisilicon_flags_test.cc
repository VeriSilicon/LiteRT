...
TEST(UpdateVerisiliconOptionsFromFlagsTest, DefaultValue) {
  ...
  EXPECT_EQ(options.Value().GetDeviceIndex(), 0);
  EXPECT_EQ(options.Value().GetCoreIndex(), 0);
  EXPECT_EQ(options.Value().GetVivanteSDKDir(), "");
}
...
TEST(UpdateVerisiliconOptionsFromFlagsTest, SetFlagToDevice1) {
  ...
  ASSERT_TRUE(options.HasValue());
  ASSERT_TRUE(UpdateVerisiliconOptionsFromFlags(options.Value()).HasValue());
  EXPECT_EQ(options.Value().GetDeviceIndex(), 1);
}
...
TEST(UpdateVerisiliconOptionsFromFlagsTest, SetFlagToCore1) {
  ...
  ASSERT_TRUE(options.HasValue());
  ASSERT_TRUE(UpdateVerisiliconOptionsFromFlags(options.Value()).HasValue());
  EXPECT_EQ(options.Value().GetCoreIndex(), 1);
}

TEST(UpdateVerisiliconOptionsFromFlagsTest, SetVivanteSDKDir) {
  absl::SetFlag(&FLAGS_verisilicon_viv_sdk_dir, "/data/local/tmp");
  Expected<VerisiliconOptions> options = VerisiliconOptions::Create();
  ASSERT_TRUE(options.HasValue());
  ASSERT_TRUE(UpdateVerisiliconOptionsFromFlags(options.Value()).HasValue());
  EXPECT_EQ(options.Value().GetVivanteSDKDir(), "/data/local/tmp");
  // Reset flag to default to avoid affecting other tests
  absl::SetFlag(&FLAGS_verisilicon_viv_sdk_dir, "");
}
