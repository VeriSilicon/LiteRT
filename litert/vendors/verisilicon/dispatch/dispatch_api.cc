...
std::unique_ptr<litert::verisilicon::VerisiliconOptions> vsi_options;
...
LiteRtStatus LiteRtInitialize(LiteRtEnvironmentOptions environment_options,
  ...
  auto [env, opts, opq_opts, verisilicon_opts] =
      litert::ParseOptions<litert::verisilicon::VerisiliconOptions>(
          environment_options, options);

  if (verisilicon_opts) {
    LITERT_LOG(LITERT_INFO, "Found Verisilicon runtime options");
  } else {
    LITERT_LOG(LITERT_INFO, "No Verisilicon runtime options found, using defaults");
    LITERT_ASSIGN_OR_RETURN(verisilicon_opts,
                            ::litert::verisilicon::VerisiliconOptions::Create());
  }
  vsi_options = std::make_unique<litert::verisilicon::VerisiliconOptions>(
    std::move(*verisilicon_opts));
  ...
  if (auto viplite_adapter_api = litert::verisilicon::VipliteAdapterApi::Create(
          shared_library_dir_opt, verisilicon_opts);
      viplite_adapter_api) {
...
LiteRtStatus LiteRtDeviceContextCreate(
  ...
  if (auto context =
          LiteRtDispatchDeviceContextT::Create(*static_viplite_adapter,vsi_options.get());
      context) {
